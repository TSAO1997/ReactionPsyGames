...
class stroop():
    ...
    def tutorial(self):
        ...
        self.canvas.create_text(400, 300, font=("Times New Roman", 20), text='舉例:出現紅色的""藍""就要選紅,所以要按下1')
        self.canvas.create_text(400, 350, font=("Times New Roman", 20), text='準備好就可以開始！')
        self.canvas.pack()
    ...
    def new_question(self):
        ...
        self.description_label.configure(text=self.word_list[w], font=self.fontsize,fg=self.correct_ans)
    ...
    def check_trial(self):
        if self.trial == 40:
            ...
            self.canvas.create_text(400, 300, font=("Times New Roman", 24), text='文字跟顏色一致時你的判斷時間 : {:.3f}'.format(incongruent_avg))
            self.canvas.create_text(400, 350, font=("Times New Roman", 24), text='史楚普效應為後者時間減去前者時間 : '.format(incongruent_avg-congruent_avg))
    ...
    def run(self):
        ...
        self.label1 = tk.Label(self.label_frame, text='1', font=self.fontsize, width=6, height=3)
        self.label1.pack(side = tk.LEFT, fill="x", expand=True)

        self.label2 = tk.Label(self.label_frame, text='2', font=self.fontsize, width=6, height=3)
        self.label2.pack(side = tk.LEFT, fill="x", expand=True)

        self.label3 = tk.Label(self.label_frame, text='3', font=self.fontsize, width=6, height=3)
        self.label3.pack(side = tk.LEFT,fill="x", expand=True)

        self.label4 = tk.Label(self.label_frame, text='4', font=self.fontsize, width=6, height=3)
        self.label4.pack(side = tk.LEFT,fill="x", expand=True)

        self.ans_frame = tk.Frame(self.window)
        self.ans_frame.pack(side=tk.TOP)
        
        self.red_label = tk.Label(self.ans_frame, text='紅', font=self.fontsize, width=6, height=3)
        self.red_label.pack(side = tk.LEFT,fill="x", expand=True)

        self.yellow_label = tk.Label(self.ans_frame, text='黃', font=self.fontsize, width=6, height=3)
        self.yellow_label.pack(side = tk.LEFT,fill="x", expand=True)

        self.green_label = tk.Label(self.ans_frame, text='綠', font=self.fontsize, width=6, height=3)
        self.green_label.pack(side = tk.LEFT,fill="x", expand=True)

        self.blue_label = tk.Label(self.ans_frame, text='藍', font=self.fontsize, width=6, height=3)
